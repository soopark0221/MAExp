        ...
        for i in type_idx:
            ...
            if self.alg_types[i] == 'Bootc':
                self.shared_agents.append(BootcAgent(lr=lr, discrete_action=discrete_action,
                                 ...
                                 **params))
            elif self.alg_types[i]=='Boota':
                self.shared_agents.append(BootaAgent(lr=lr, discrete_action=discrete_action,
                    hidden_dim=hidden_dim,
                    **params))
            else:
                self.shared_agents.append(DDPGAgent(lr=lr, discrete_action=discrete_action,
                                 hidden_dim=hidden_dim,
                                 **params))
        
        if len(type_idx)==1:
            self.agents=[self.shared_agents[0] for _ in range(self.nagents)]
        else:
            self.agents=[self.shared_agents[0] if agent=='adversary' else self.shared_agents[1] for agent in agent_types]
