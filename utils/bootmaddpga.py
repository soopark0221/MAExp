...
class BootaAgent(object):
    ...
    def __init__(self, num_in_pol, num_out_pol, num_in_critic, hidden_dim=64,
        ...
        self.policy = torch.nn.ModuleList([MLPNetwork(num_in_pol, num_out_pol,
                                 ...
                                 constrain_out=True,
                                 discrete_action=discrete_action) for _ in range(n_ensemble)])
        self.critic = MLPNetwork(num_in_critic, 1,
                                 hidden_dim=hidden_dim,
                                 constrain_out=False)
        self.target_policy = torch.nn.ModuleList([MLPNetwork(num_in_pol, num_out_pol,
                                 hidden_dim=hidden_dim,
                                 constrain_out=True,
                                 discrete_action=discrete_action) for _ in range(n_ensemble)])
