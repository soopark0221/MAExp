...
class BootaBuffer(ReplayBuffer):
    ...
    def push(self, observations, actions, rewards, next_observations, dones, actor_idx):
        ...
        if self.curr_i + nentries > self.max_steps:
            ...
            for agent_i in range(self.num_agents):
                ...
                self.done_buffs[agent_i] = np.roll(self.done_buffs[agent_i],
                                                   rollover)
                self.actor_buffs[agent_i]=np.roll(self.actor_buffs[agent_i], rollover)
    ...
    def sample_boot(self, N, to_gpu=False, norm_rews=True):
        inds = np.random.choice(np.arange(self.filled_i), size=N,
                                replace=False)
        if to_gpu:
            cast = lambda x: Variable(Tensor(x), requires_grad=False).cuda()
        ...
        return ([cast(self.obs_buffs[i][inds]) for i in range(self.num_agents)],
                ...
                [cast(self.next_obs_buffs[i][inds]) for i in range(self.num_agents)],
                [cast(self.done_buffs[i][inds]) for i in range(self.num_agents)],
                [cast(self.actor_buffs[i][inds]) for i in range(self.num_agents)]) # agent,batch , 1(actor idx)